...
for (const typeMaterial of miCamino.material) {
    if (typeMaterial.nombre.includes("Piolets")) {
      console.log(`Lo primero que debo llevar es ${typeMaterial.nombre} y es ${typeMaterial.descripcion}.`);

    }  else if (typeMaterial.nombre.includes("Crampones")){
      console.log(`Lo segundo que debo llevar es ${typeMaterial.nombre} y es ${typeMaterial.descripcion}.`);

    }  else if (typeMaterial.nombre.includes("Cuerdas")){
      console.log(`Lo tercero que debo llevar es ${typeMaterial.nombre} y es ${typeMaterial.descripcion}.`);

    } else {
      console.log(`Y por ultimo debo llevar la ${typeMaterial.nombre} que es ${typeMaterial.descripcion}.`);
  }
}
...
for (let i = 0; i < miCamino.subiendoAlEverest[0][0].campoBase[0].campoI.campoII.campoIII.campoIV.length; i++) {
  ...
  let element = miCamino.subiendoAlEverest[0][0].campoBase[0].campoI.campoII.campoIII.campoIV[i];

   if(typeof element === "string"){

      fraseAnimo.push(element);
   }
  }
  console.log(fraseAnimo.join(" "));
